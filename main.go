...
func main() {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:   "02de9665.cluster.x-k8s.io",
	})
	if err != nil {
	...
	if err = (&expinfrastructurecontroller.HyperMachinePoolReconciler{
		Client: mgr.GetClient(),
		Log:    ctrl.Log.WithName("controllers").WithName("HyperMachinePool"),
	...
	if err = (&infrastructurecontroller.HyperMachineReconciler{
		Client: mgr.GetClient(),
		Log:    ctrl.Log.WithName("controllers").WithName("HyperMachine"),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "HyperMachine")
		os.Exit(1)
	}
	if err = (&expinfrastructurecontroller.HyperIPPoolReconciler{
		Client: mgr.GetClient(),
		Log:    ctrl.Log.WithName("controllers").WithName("HyperIPPool"),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "HyperIPPool")
		os.Exit(1)
	}
